...
class Aside extends HTag {
	...
	def dispatch CharSequence content_Element(Void self) ''''''
	...
	def dispatch CharSequence content_Element(Site self) '''
		«self.nav»
		«self.para_Site_tags»
		«self.para_Site_abbreviations»
	'''

	def dispatch CharSequence content_Element(Topic self) '''
		<ul class='nav nav-list'>
